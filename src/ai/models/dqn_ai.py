...
class DQNAgent(AIPlayer):
    ...
    def _train_tactical(self) -> float | None:
        ...
        if self.use_prioritized_replay:
            ...
            experiences, weights, indices = sampled_data  # type: ignore[misc]
            experiences = list(experiences)  # Ensure proper type
            weights = torch.FloatTensor(weights).to(device)
        else:
            experiences = list(self.memory.sample(self.batch_size))  # type: ignore[arg-type]
            weights = torch.ones(len(experiences)).to(device)
            indices = None

        # Compute tactical loss
        loss = self._compute_dqn_loss(experiences, weights)  # type: ignore[arg-type]
        ...
        if self.use_prioritized_replay and indices is not None:
            ...
            with torch.no_grad():
                td_errors = self._compute_td_errors(experiences)  # type: ignore[arg-type]
                self.memory.update_priorities(indices, td_errors.cpu().numpy())  # type: ignore[union-attr]
    ...
    def _compute_td_errors(self, experiences: list[Transition]) -> torch.Tensor:
        ...
        states = torch.FloatTensor(np.array([e.state for e in experiences])).to(device)  # type: ignore[attr-defined]
        actions = torch.LongTensor([e.action for e in experiences]).to(device)  # type: ignore[attr-defined]
        rewards = torch.FloatTensor([e.reward for e in experiences]).to(device)  # type: ignore[attr-defined]
        next_states = torch.FloatTensor(np.array([e.next_state for e in experiences])).to(device)  # type: ignore[attr-defined]
        dones = torch.BoolTensor([e.done for e in experiences]).to(device)  # type: ignore[attr-defined]
    ...
    def _calculate_discounted_rewards(self, rewards: list[float]) -> list[float]:
        ...
        if self.reward_normalization and len(discounted_rewards) > 1:
            ...
            if std_reward > 1e-8:  # Avoid division by zero
                discounted_rewards = [(r - mean_reward) / std_reward for r in discounted_rewards]
    ...
    def replay(self) -> float | None:
        ...
        # Vérification rapide de la taille du buffer
        if len(self.memory) < self.batch_size:
            return None
