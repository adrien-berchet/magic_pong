...
from magic_pong.core.entities import Action, GameState
...
class AIPlayer(ABC):
    ...
    def get_stats(self) -> Dict[str, float]:
        ...
        return {
            'mean_reward': float(np.mean(self.episode_rewards[-100:])),  # Moyenne sur 100 derniers épisodes
...
class ObservationProcessor:
    ...
    def process_game_state(self, game_state: Dict[str, Any], player_id: int) -> Dict[str, Any]:
        ...
        if ai_config.INCLUDE_VELOCITY:
            if ai_config.NORMALIZE_POSITIONS:
                vel_x = game_state['ball_velocity'][0] / 500.0  # Normaliser par vitesse max approximative
...
class RewardCalculator:
    ...
    def calculate_reward(self, game_state: Dict[str, Any], events: Dict[str, List],
                        player_id: int) -> float:
        """
...
class GameEnvironment:
    ...
    def __init__(self, physics_engine, headless: bool = False):
        self.physics_engine = physics_engine
        self.headless = headless

        self.observation_processor = ObservationProcessor(
            physics_engine.field_width,
            physics_engine.field_height
        )
    ...
    def step(self, action1: Optional[Action], action2: Optional[Action]) -> Tuple[
        Dict[str, Any], Dict[str, Any], float, float, bool, Dict[str, Any]
    ]:
        """
        ...
        done = (self.physics_engine.is_game_over() or
